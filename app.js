...
function setGreeting() {
  ...
  // Check if custom hero title configured by admin
  try {
    ...
    if (raw) {
      ...
      if (cfg.heroTitle && cfg.heroTitle.trim() && cfg.heroTitle.trim() !== 'Good Evening') {
        textEl.textContent = cfg.heroTitle.trim();
        return;
      }
    }
  } catch (e) { }
...
function navigateTo(section, pushHistory = true) {
  ...
  if (!target) {
    console.warn(`Section "section-${section}" not found, falling back to home`);
    section = 'home';
    target = document.getElementById('section-home');
  }
